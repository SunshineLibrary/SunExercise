...
public class WebInteraction implements SyncObserver{
    ...
    public void onCollectionDownloaded(final String collectionId, final boolean available) {
        mActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                mWebView.loadUrl("javascript:Interfaces.onCollectionDownloaded('" + collectionId + "', '" + available + "')");
            }
        });
    }
    ...
    public void onAudioComplete(final String id){
        mActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() { mWebView.loadUrl("javascript:Interfaces.onAudioComplete('" + id + "')");}
        });
    }
}
